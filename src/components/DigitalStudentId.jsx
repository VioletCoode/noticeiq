...
export default function DigitalStudentId({ 
  ...
  // Active social links: merge props from CampusVault with direct DB profile links and custom_links
  const dedicatedSocials = [
    ...
    { id: 'twitter', name: 'Twitter / X', iconType: 'twitter', url: profiles.find((p) => p.id === 'twitter')?.url || dbProfile?.twitter_url || '' },
    { id: 'instagram', name: 'Instagram', iconType: 'instagram', url: profiles.find((p) => p.id === 'instagram')?.url || dbProfile?.instagram_url || '' },
    { id: 'youtube', name: 'YouTube', iconType: 'youtube', url: profiles.find((p) => p.id === 'youtube')?.url || dbProfile?.youtube_url || '' },
    { id: 'discord', name: 'Discord', iconType: 'discord', url: profiles.find((p) => p.id === 'discord')?.url || dbProfile?.discord_url || '' },
  ...
  const customSocials = Array.isArray(dbProfile?.custom_links)
    ? dbProfile.custom_links
  ...
  const activeSocials = [...dedicatedSocials, ...customSocials];
  ...
  const handleProfileSaved = (updated) => {
    ...
    setDbProfile((prev) => ({
      ...
      youtube_url: updated.youtubeUrl || null,
      twitter_url: updated.twitterUrl || null,
      discord_url: updated.discordUrl || null,
      portfolio_url: updated.portfolioUrl || null,
      leetcode_url: updated.leetcodeUrl || null,
      kaggle_url: updated.kaggleUrl || null,
    ...
    <div className="w-full space-y-3">
      ...
      <EditProfileModal
        ...
        initialData={{
          ...
          twitterUrl: dbProfile?.twitter_url || '',
          discordUrl: dbProfile?.discord_url || '',
          portfolioUrl: dbProfile?.portfolio_url || profiles.find((p) => p.id === 'portfolio')?.url || '',
          leetcodeUrl: dbProfile?.leetcode_url || profiles.find((p) => p.id === 'leetcode')?.url || '',
          kaggleUrl: dbProfile?.kaggle_url || profiles.find((p) => p.id === 'kaggle')?.url || '',
          customLinks: dbProfile?.custom_links || []
        }}
